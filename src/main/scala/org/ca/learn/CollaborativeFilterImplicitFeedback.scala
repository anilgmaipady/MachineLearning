...
object CollaborativeFilterImplicitFeedback {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val songs = spark.read.option("delimiter"," ").option("inferschema","true").csv("./src/main/scala/resource/kaggle_songs.txt").toDF("song","songId")
    songs.show()
    ...
    val triplets1 = spark.read.option("delimiter",",").option("inferschema","true").csv("./src/main/scala/resource/kaggle_visible_evaluation_triplets.txt")
    triplets1.show()
